...
pub const SQL_UPDATE_AGENT_TASK: &str =
    "UPDATE ai_agent_task SET title = $1, prompt = $2, status = $3, external_ref = $4, metadata_json = $5::jsonb, version = $6, updated_at = $7::timestamptz, started_at = $8::timestamptz, completed_at = $9::timestamptz, cancelled_at = $10::timestamptz WHERE tenant_id = $11 AND organization_id = $12 AND task_id = $13 AND version = $14";
#[cfg(feature = "postgres-sync")]
pub const SQL_SELECT_AGENT_TASK: &str =
    "SELECT id, uuid, tenant_id, organization_id, agent_id, task_id, owner_user_id, title, prompt, status, external_ref, metadata_json::text AS metadata_json, version, created_at::text AS created_at, updated_at::text AS updated_at, started_at::text AS started_at, completed_at::text AS completed_at, cancelled_at::text AS cancelled_at FROM ai_agent_task WHERE tenant_id = $1 AND organization_id = $2 AND task_id = $3 LIMIT 1";
#[cfg(feature = "postgres-sync")]
pub const SQL_LIST_AGENT_TASKS: &str =
    "SELECT id, uuid, tenant_id, organization_id, agent_id, task_id, owner_user_id, title, prompt, status, external_ref, metadata_json::text AS metadata_json, version, created_at::text AS created_at, updated_at::text AS updated_at, started_at::text AS started_at, completed_at::text AS completed_at, cancelled_at::text AS cancelled_at FROM ai_agent_task WHERE tenant_id = $1 AND organization_id = $2 AND ($3::text IS NULL OR agent_id = $3) AND ($4::bigint IS NULL OR owner_user_id = $4) AND ($5::smallint IS NULL OR status = $5) ORDER BY updated_at DESC LIMIT $6 OFFSET $7";
#[cfg(feature = "postgres-sync")]
pub const SQL_COUNT_AGENT_TASKS: &str =
    "SELECT COUNT(*)::bigint AS total_count FROM ai_agent_task WHERE tenant_id = $1 AND organization_id = $2 AND ($3::text IS NULL OR agent_id = $3) AND ($4::bigint IS NULL OR owner_user_id = $4) AND ($5::smallint IS NULL OR status = $5)";
