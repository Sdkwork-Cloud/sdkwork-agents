use crate::domain::{
    ...
    AgentMcpAuthKind, AgentMcpServerRecord, AgentMcpTransportKind, AgentMemoryRecord, AgentCompositionSlotRecord, AgentCompositionSlotKind, AgentCompositionTargetModule, AgentProviderBindingRecord, AgentRuntimeExecutionOperation, AgentRuntimeExecutionRecord,
    ...
    P: PolicyProvider,
{
    ...
    fn get_active_knowledge_base(
        &self,
        tenant_id: u64,
        knowledge_base_id: &str) -> KernelResult<> {
        let record = self
            .repository
    ...
    fn get_active_memory_store(
        &self,
        tenant_id: u64,
        memory_store_id: &str) -> KernelResult<> {
        let record = self
            .repository
    ...
    fn get_active_memory_profile(
        &self,
        tenant_id: u64,
        memory_profile_id: &str) -> KernelResult<> {
        let record = self
            .repository
    ...
    fn get_active_memory_namespace(
        &self,
        tenant_id: u64,
        memory_namespace_id: &str) -> KernelResult<> {
        let record = self
            .repository
    ...
    fn get_active_memory_record(
        &self,
        tenant_id: u64,
        memory_id: &str) -> KernelResult<AgentMemoryRecord> {
        let record = self
            .repository
    ...
    fn get_active_knowledge_source(
        &self,
        tenant_id: u64,
        knowledge_source_id: &str) -> KernelResult<> {
        let record = self
            .repository
    ...
    fn get_active_knowledge_document(
        &self,
        tenant_id: u64,
        knowledge_document_id: &str) -> KernelResult<> {
        let record = self
            .repository
    ...
    fn get_active_knowledge_chunk(
        &self,
        tenant_id: u64,
        knowledge_chunk_id: &str) -> KernelResult<> {
        let record = self
            .repository
...
fn knowledge_sync_job_audit_sequence(status: ) -> u64 {
    match status {
        ::Queued => 1,
        ::Running => 2,
        ::Succeeded => 3,
...
fn validate_optional_standard_ref(
    value: Option<&str>,
    field_name: &str,
    required_prefix: &str) -> KernelResult<()> {
...
fn validate_mcp_transport_reference_pair(
    ...
    command_ref: Option<&str>) -> KernelResult<()> {
...
fn validate_non_empty_memory_modes(values: &[]) -> KernelResult<()> {
    if values.is_empty() {
        return Err(KernelError::validation(
...
fn validate_non_empty_knowledge_modes(values: &[]) -> KernelResult<()> {
    if values.is_empty() {
        return Err(KernelError::validation(
            "retrievalModes must contain at least one mode"));
    }
