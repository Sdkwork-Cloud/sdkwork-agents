...
Production deployments mount raw route builders from `sdkwork-intelligence-agents-service` through route
boundary crates:

- `sdkwork-routes-agents-app-api::build_served_router`
- `sdkwork-routes-agents-backend-api::build_served_router`
- `sdkwork-routes-agents-open-api::build_served_router`
- `sdkwork-routes-agents-http-shared::build_served_combined_router` (all surfaces)
...
`sdkwork-routes-agents-http-shared/src/web_bootstrap.rs`.
...
- Local development: route-crate web-framework tests use dev inline dual tokens; legacy contract tests use matching `tenant_id=100001` and `x-subject-tenant-id: 100001` headers.

## Verification

- Legacy gateway contracts: `cargo test --features http-axum --manifest-path sdkwork-intelligence-agents-service/Cargo.toml`
- Served web-framework contracts: `cargo test -p sdkwork-routes-agents-app-api` (and backend/open route crates)
