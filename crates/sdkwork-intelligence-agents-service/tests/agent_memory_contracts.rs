use sdkwork_intelligence_agents_service::{
    AgentsService, AgentBusinessStatus, AgentMemoryBindingCreateCommand,
...
fn subject() -> PolicySubject {
    PolicySubject::new("user.memory.admin", "100001").with_role("agent.memory.admin")
}

fn service(
) -> AgentsService<InMemoryAgentRepository, InMemoryAgentAuditSink, AllowAllPolicyProvider> {
    AgentsService::new(
...
fn memory_business_crud_surface_is_documented_as_current_contract() {
    let database_spec = include_str!("../specs/AGENTS_MANAGED_STORE_DATABASE_SPEC.md");
...
fn memory_store_profile_binding_namespace_and_record_crud_form_standard_memory_stack() {
    ...
    let listed = service
        .list_memory_records(100_001, "memory.namespace.agent.alpha.user.1", subject())
...
fn memory_source_relation_and_retrieval_index_support_non_vector_rag_and_provenance() {
    ...
    let sources = service
        .list_memory_sources(100_001, "memory.record.fact.project", subject())
    ...
    let relations = service
        .list_memory_relations(100_001, "memory.record.fact.project", subject())
    ...
    let indexes = service
        .list_memory_retrieval_indexes(100_001, "memory.record.fact.project", subject())
...
fn memory_child_resources_require_active_parent_records() {
    ...
    let list_error = service
        .list_memory_retrieval_indexes(100_001, "memory.record.deleted.parent", subject())
