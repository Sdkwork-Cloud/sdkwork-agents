use sdkwork_intelligence_agents_service::{
    AgentBusinessIdGenerator, AgentsService, AgentIdGenerator, AgentMcpAuthKind,
...
fn subject() -> PolicySubject {
    PolicySubject::new("user.id.admin", "100001").with_role("agent.id.admin")
}
...
fn runtime_services_assign_snowflake_ids_instead_of_local_counters() {
    let mut service = AgentsService::new(
