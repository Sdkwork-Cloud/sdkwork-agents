...
fn live_postgres_memory_relation_get_roundtrip_when_uri_configured() {
    let Some(_uri) = std::env::var("SDKWORK_AGENTS_STORE_POSTGRES_URI")
        .ok()
        .filter(|value| !value.trim().is_empty())
    else {
        eprintln!(
            "skipping live postgres sync test: SDKWORK_AGENTS_STORE_POSTGRES_URI is not set"
        );
        return;
    };
    ...
    let loaded = repository
        .get_memory_relation(100_001, memory_relation_id.as_str())
