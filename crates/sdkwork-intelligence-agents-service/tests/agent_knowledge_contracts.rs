use sdkwork_intelligence_agents_service::{
    AgentsService, AgentBusinessStatus, AgentKnowledgeBaseCreateCommand,
...
fn subject() -> PolicySubject {
    PolicySubject::new("user.knowledge.admin", "100001").with_role("agent.knowledge.admin")
}
...
fn knowledge_base_source_document_chunk_index_binding_and_sync_job_form_standard_rag_stack() {
    ...
    let indexes = service
        .list_knowledge_indexes(100_001, "knowledge.document.kernel.spi", subject())
...
fn knowledge_sources_support_retrieve_update_delete_and_restore_lifecycle() {
    ...
    assert!(deleted.deleted_at.is_some());
    assert!(service
        .list_knowledge_sources(100_001, "knowledge.base.sources.crud", subject())
...
fn knowledge_child_operations_treat_soft_deleted_documents_as_not_found() {
    ...
    let chunks_after_delete = service
        .list_knowledge_chunks(100_001, "knowledge.document.lifecycle.deleted", subject())
    ...
    let indexes_after_delete = service
        .list_knowledge_indexes(100_001, "knowledge.document.lifecycle.deleted", subject())
