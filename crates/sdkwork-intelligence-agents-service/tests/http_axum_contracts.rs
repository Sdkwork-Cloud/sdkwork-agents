...
async fn app_create_agent_should_derive_scope_from_request_context() {
    ...
    assert_eq!(response["data"]["tenantId"], "100001");
    assert_eq!(response["data"]["organizationId"], "0");
...
async fn provider_bindings_and_deployments_should_work_over_http() {
    ...
    let create_deployment_request = Request::builder()
        .method("POST")
        .uri("/backend/v3/api/ai/agents/agent.rig.http/deployments?tenant_id=100001")
        .header(CONTENT_TYPE, "application/json")
        .body(Body::from(
            json!({
...
async fn provider_bindings_and_deployments_should_apply_pagination_contract() {
    ...
    for (deployment_id, binding_id, requested_at) in [
        ...
        let request = Request::builder()
            .method("POST")
            .uri("/backend/v3/api/ai/agents/agent.rig.paged/deployments?tenant_id=100001")
            .header(CONTENT_TYPE, "application/json")
            .body(Body::from(
                json!({
    ...
    let request = Request::builder()
        .method("GET")
        .uri("/backend/v3/api/ai/agents/agent.rig.paged/deployments?tenant_id=100001&page=2&page_size=1")
        .body(Body::empty())
        .expect("request should be built");
    let response = app
        .clone()
...
async fn provider_binding_and_deployment_list_missing_agent_should_return_not_found() {
    ...
    for uri in [
        ...
        "/backend/v3/api/ai/agents/agent.missing/deployments?tenant_id=100001",
    ] {
        let request = Request::builder()
            .method("GET")
            .uri(uri)
...
async fn provider_binding_and_deployment_conflicts_should_return_problem_detail() {
    ...
    for expected_status in [StatusCode::CREATED, StatusCode::CONFLICT] {
        let request = Request::builder()
            .method("POST")
            .uri("/backend/v3/api/ai/agents/agent.rig.conflict/deployments?tenant_id=100001")
            .header(CONTENT_TYPE, "application/json")
...
async fn deployment_missing_binding_should_return_not_found_problem_detail() {
    ...
    let request = Request::builder()
        .method("POST")
        .uri("/backend/v3/api/ai/agents/agent.rig.missing.binding/deployments?tenant_id=100001")
        .header(CONTENT_TYPE, "application/json")
        .body(Body::from(
            json!({
...
async fn deployment_invalid_standard_ids_should_return_bad_request() {
    ...
    let request = Request::builder()
        .method("POST")
        .uri("/backend/v3/api/ai/agents/agent.rig.invalid.deployment/deployments?tenant_id=100001")
        .header(CONTENT_TYPE, "application/json")
        .body(Body::from(
            json!({
...
async fn backend_audit_events_should_filter_provider_binding_and_deployment_actions() {
    ...
    let deployment_request = Request::builder()
        .method("POST")
        .uri("/backend/v3/api/ai/agents/agent.audit.rig/deployments?tenant_id=100001")
        .header(CONTENT_TYPE, "application/json")
        .body(Body::from(
            json!({
