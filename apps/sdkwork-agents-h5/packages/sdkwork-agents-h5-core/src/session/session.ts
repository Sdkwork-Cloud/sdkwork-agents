...
function readPersistedSessionRawValue(): string | null {
  ...
  const durableValue = localStorage?.getItem(SDKWORK_AGENTS_H5_SESSION_KEY) ?? null;
  if (durableValue) {
    persistedSessionRawValueCache = durableValue;
    return durableValue;
  }

  const legacyValue = getSessionStorage()?.getItem(SDKWORK_AGENTS_H5_SESSION_KEY) ?? null;
  if (legacyValue) {
    localStorage?.setItem(SDKWORK_AGENTS_H5_SESSION_KEY, legacyValue);
...
function writePersistedSessionRawValue(value: string): void {
  ...
  getLocalStorage()?.setItem(SDKWORK_AGENTS_H5_SESSION_KEY, value);
  getSessionStorage()?.removeItem(SDKWORK_AGENTS_H5_SESSION_KEY);
}
...
function migrateLegacySessionStorage(): void {
  ...
  const legacyValue = sessionStorage.getItem(SDKWORK_AGENTS_H5_SESSION_KEY);
  if (!legacyValue || localStorage.getItem(SDKWORK_AGENTS_H5_SESSION_KEY)) {
    return;
  }

  localStorage.setItem(SDKWORK_AGENTS_H5_SESSION_KEY, legacyValue);
  sessionStorage.removeItem(SDKWORK_AGENTS_H5_SESSION_KEY);
}
