...
const SDKWORK_AGENTS_H5_SESSION_KEY = 'sdkwork-agents-h5:session:v1';
const SDKWORK_AGENTS_H5_LEGACY_SESSION_STORAGE_KEY = 'sdkwork-agents-h5:session:v1';
export const SDKWORK_AGENTS_H5_SESSION_CHANGED_EVENT = 'sdkwork-agents-h5:auth-session-changed';
...
function readPersistedSessionRawValue(): string | null {
  ...
  const fromSession = sessionStorage?.getItem(SDKWORK_AGENTS_H5_SESSION_KEY);
  if (fromSession) {
    persistedSessionRawValueCache = fromSession;
    return fromSession;
  }

  const legacyValue = getLocalStorage()?.getItem(SDKWORK_AGENTS_H5_LEGACY_SESSION_STORAGE_KEY) ?? null;
...
function writePersistedSessionRawValue(value: string): void {
  ...
  getSessionStorage()?.setItem(SDKWORK_AGENTS_H5_SESSION_KEY, value);
  getLocalStorage()?.removeItem(SDKWORK_AGENTS_H5_LEGACY_SESSION_STORAGE_KEY);
}
...
function removePersistedSessionRawValue(): void {
  ...
  getSessionStorage()?.removeItem(SDKWORK_AGENTS_H5_SESSION_KEY);
  getLocalStorage()?.removeItem(SDKWORK_AGENTS_H5_LEGACY_SESSION_STORAGE_KEY);
}
...
export async function hydrateAppSdkSessionFromSecureStorage(): Promise<void> {
  ...
  const legacyValue = getSessionStorage()?.getItem(SDKWORK_AGENTS_H5_SESSION_KEY)
    ?? getLocalStorage()?.getItem(SDKWORK_AGENTS_H5_LEGACY_SESSION_STORAGE_KEY)
    ?? null;
  if (!legacyValue) {
    persistedSessionRawValueCache = null;
    return;
...
function migrateLegacySessionStorage(): void {
  ...
  const legacyValue = localStorage.getItem(SDKWORK_AGENTS_H5_LEGACY_SESSION_STORAGE_KEY);
  if (!legacyValue || sessionStorage.getItem(SDKWORK_AGENTS_H5_SESSION_KEY)) {
    return;
  }

  sessionStorage.setItem(SDKWORK_AGENTS_H5_SESSION_KEY, legacyValue);
  localStorage.removeItem(SDKWORK_AGENTS_H5_LEGACY_SESSION_STORAGE_KEY);
}
...
function dispatchAppSdkSessionChanged(session: SdkworkChatSession | null): void {
  ...
  window.dispatchEvent(new CustomEvent(
    SDKWORK_AGENTS_H5_SESSION_CHANGED_EVENT,
    { detail: { session } },
  ));
}
...
export function normalizeSdkworkChatSessionUser(value: unknown): SdkworkChatSessionUser | undefined {
  ...
  const chatId = normalizeString(userRecord.chatId)
    ...
    ?? normalizeString(userRecord.SDKWORK_AGENTS_H5_id);
