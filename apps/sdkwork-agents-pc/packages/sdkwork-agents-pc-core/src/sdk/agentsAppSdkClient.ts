...
export function resolveAgentsAppSdkBaseUrl(): string {
  const fromEnv = import.meta.env.VITE_SDKWORK_AGENTS_PC_APP_API_BASE_URL;
  if (typeof fromEnv === "string" && fromEnv.trim().length > 0) {
    return fromEnv.trim();
  }
  const publicUrl = import.meta.env.VITE_SDKWORK_AGENTS_PC_APPLICATION_PUBLIC_HTTP_URL ?? "http://127.0.0.1:8095";
