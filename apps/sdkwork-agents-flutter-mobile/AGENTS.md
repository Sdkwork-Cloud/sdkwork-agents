...
Read `sdkwork.app.config.json` when changing Flutter mobile behavior, runtime config, or SDK wiring. Managed-agent APIs are owned by repository root `sdkwork-agents`.
...
```powershell
cd apps/sdkwork-agents-flutter-mobile
flutter test
pnpm verify
```
